...
summit_files <- list.files("output/peaks/", pattern = 'highCells_summits.bed$', full.names = T)
pileup_files <- list.files("output/peaks/", pattern = 'highCells_peaks.xls$', full.names = T)
plot_dir <- 'output/merged_peaks/plots/'
bed_dir <- 'output/merged_peaks/beds/'
...
find_reproducible_summits <- function(sampleName, distanceCutoff = 75, minPileup = 10) {
  ...
  combined_summits$donor <- str_extract(string = combined_summits$name, pattern = 'Donor[A-Z]')
...
#############################################################
# Output summits around CTLA4 for testing
subset_range <- GRanges(seqnames = 'chr2', IRanges(203817771, 203873965))
# export(subsetByOverlaps(all_summits, subset_range), paste0(bed_dir, 'CTLA4region_summits.bed'))
export(subsetByOverlaps(all_summits, subset_range), paste0(bed_dir, 'CTLA4region_summits_highCells.bed'))
list.files(bed_dir)
...
save_plot(paste0(plot_dir, 'all_summits_gap_distance_highCells.pdf'), all_summits_gap_distance_plot, base_height = 10)
...
export(initial_summit_clusters_filtered, paste0(bed_dir, 'initial_filtered_clusters_highCells.bed'))
...
write_rds(x = all_peaks, path = paste0(bed_dir, 'all_peaks_highCells.RDS'))
