...
tss_file="hg38_gencode_tss_unique.bed"
window_size <- 2000
insertion_files = list.files("output/beds/", pattern = 'highCells.insertions.bed$', full.names = T)

for(atac_file in insertion_files){
  sample <-  str_extract(atac_file, 'Donor[A-Z]_[[:graph:]]')
  plot_file <- paste0("output/qc/plots/tss/",sample,"_tss_enrichment.pdf")
  enrichment_file <- paste0("output/qc/tss/",sample,"_tss_enrichment.csv")
